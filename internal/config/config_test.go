...
func TestSaveUsesOwnerOnlyPermissions(t *testing.T) {
	...
	info, err := os.Stat(filepath.Join(home, ".config", "dbkit", "config.json"))
	if err != nil {
...
func TestLoadPrefersLegacyWhenNewConfigIsEmpty(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(home, ".config", "dbkit"), configDirMode); err != nil {
		t.Fatalf("MkdirAll new config dir: %v", err)
	}
	if err := os.MkdirAll(filepath.Join(home, ".dbkit"), configDirMode); err != nil {
		t.Fatalf("MkdirAll legacy config dir: %v", err)
	}

	if err := os.WriteFile(filepath.Join(home, ".config", "dbkit", "config.json"), []byte("{\"connections\":[]}"), configFileMode); err != nil {
		t.Fatalf("WriteFile new config: %v", err)
	}
	legacyData := []byte("{\"connections\":[{\"id\":\"p1\",\"name\":\"legacy\",\"type\":\"postgres\",\"dsn\":\"postgres://example\"}]}")
	if err := os.WriteFile(filepath.Join(home, ".dbkit", "config.json"), legacyData, configFileMode); err != nil {
